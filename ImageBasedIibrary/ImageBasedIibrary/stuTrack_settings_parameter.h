...
typedef struct 	_StuITRACK_Params
{
	...
	unsigned int stuTrack_direct_range;					//起立时允许的角度偏离范围
	float stuTrack_move_threshold;				//判定是移动目标的偏离阈值（比值）
	unsigned int stuTrack_standCount_threshold;			//判定为起立的帧数阈值
	unsigned int stuTrack_sitdownCount_threshold;		//判定为坐下的帧数阈值
	unsigned int stuTrack_moveDelayed_threshold;		//移动目标保持跟踪的延时，超过这个时间无运动，则放弃跟踪(单位：毫秒)
}StuITRACK_Params;
